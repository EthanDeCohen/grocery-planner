...
# --------------------------------------------------------------------------- #
def test_90_lb_client_resolves_to_about_65_g_per_day(conn):
    # 90 lb -> 40.8233 kg -> * 1.6 default factor = 65.317 g/day.
    c = Customer.create("Jamie", weight=90, weight_unit="lb")
    target = protein_target_for(c, conn=conn)
    assert target.daily_grams == pytest.approx(65.317, abs=0.01)
...
def test_90_kg_client_resolves_to_144_g_per_day(conn):
    ...
    target = protein_target_for(c, conn=conn)
    assert target.daily_grams == pytest.approx(144.0, abs=1e-9)
...
def test_protein_target_resolves_a_soft_deleted_customer(conn):
    ...
    assert target.daily_grams == pytest.approx(104.0, abs=1e-9)
...
def test_weekly_grams_is_seven_times_daily(conn):
    ...
    assert target.weekly_grams == pytest.approx(65 * 1.6 * 7, abs=1e-9)
...
# --------------------------------------------------------------------------- #
def test_default_protein_factor_is_1_6(conn):
    c = Customer.create("Jamie", weight=100, weight_unit="kg")
    target = protein_target_for(c, conn=conn)
    assert target.daily_grams == pytest.approx(160.0, abs=1e-9)
...
def test_per_client_protein_factor_changes_the_target(conn):
    ...
    high = Customer.create("High Factor", weight=100, weight_unit="kg", protein_factor=2.2)
    low = Customer.create("Low Factor", weight=100, weight_unit="kg", protein_factor=1.2)
    assert protein_target_for(high, conn=conn).daily_grams == pytest.approx(220.0, abs=1e-9)
    assert protein_target_for(low, conn=conn).daily_grams == pytest.approx(120.0, abs=1e-9)
...
def test_protein_factor_never_hard_coded_a_saved_formula_overrides_it(conn):
    ...
    c = Customer.create("Jamie", weight=65, weight_unit="kg")  # protein_factor default 1.6
    target = protein_target_for(c, conn=conn)
    assert target.daily_grams == pytest.approx(65 * 1.6 + 20, abs=1e-9)
...
def test_saved_formula_can_reference_profile_values(conn):
    ...
    c = Customer.create("Jamie", weight=65, weight_unit="kg")
    target = protein_target_for(c, conn=conn)
    assert target.daily_grams == pytest.approx(65 * 1.6 + 15, abs=1e-9)
...
def test_falls_back_to_default_expression_without_a_saved_formula(conn):
    ...
    c = Customer.create("Jamie", weight=65, weight_unit="kg")
    target = protein_target_for(c, conn=conn)
    assert target.daily_grams == pytest.approx(65 * 1.6, abs=1e-9)
    assert DEFAULT_FORMULA_EXPRESSION == "weight_kg * protein_factor"
...
def test_protein_target_for_only_reads_weight_kg_not_weight_display(conn):
    ...
    target = protein_target_for(c, conn=conn)
    assert target.daily_grams != pytest.approx(90 * 1.6, abs=1.0)
    assert target.daily_grams == pytest.approx(c.weight_kg * 1.6, abs=1e-9)
