...
from grocery_planner.scrapers import sprouts


def panel(protein=10.0, serving="1 BAR  40 Gram", servings="12"):
    return sprouts.Nutrition(
...
def test_serving_grams_reads_every_metric_shape(text):
    ...
    assert sprouts.serving_grams(text) == pytest.approx(227.0, rel=0.01)


def test_serving_grams_prefers_metric_over_imperial():
    """When a label prints both, the metric figure is the one it rounded to."""
    both = sprouts.serving_grams("3 OZ  85 Gram")
    imperial_only = sprouts.serving_grams("3 OZ")
    assert both == 85.0
...
def test_serving_grams_never_reads_a_volume_as_a_mass(text):
    ...
    """
    assert sprouts.serving_grams(text) is None
...
def test_size_is_weight_recognises_rate_denominations(size):
    assert sprouts.size_is_weight(size) is True
...
def test_size_is_weight_leaves_package_quantities_alone(size):
    ...
    assert sprouts.size_is_weight(size) is False
...
def test_per_lb_item_never_multiplies_by_servings_per_container():
    ...
    assert sprouts.protein_per_100g(facts, "per lb") == pytest.approx(by_serving)
    ...
    no_mass = sprouts.Nutrition(21.0, "PER CONTAINER", "30")
    assert sprouts.protein_per_100g(no_mass, "per lb") is None


def test_package_grams_refuses_a_rate_denomination():
    """'per lb' describes what a dollar buys, not what the package weighs."""
    assert sprouts.package_grams("per lb") is None
    assert sprouts.package_grams("14 oz") == pytest.approx(396.9, rel=0.01)
...
def test_both_density_routes_agree_on_a_consistent_label():
    ...
    without_mass = sprouts.Nutrition(10.0, "1 BAR", "12")

    route_one = sprouts.protein_per_100g(with_mass, "16.93 oz")
    route_two = sprouts.protein_per_100g(without_mass, "16.93 oz")
...
def test_density_is_none_when_neither_route_is_available():
    ...
    facts = sprouts.Nutrition(10.0, "PER CONTAINER", "Varied")
    assert sprouts.protein_per_100g(facts, "1 ct") is None


@pytest.mark.parametrize("protein", [None, 0.0, -1.0])
def test_density_requires_a_positive_protein_figure(protein):
    assert sprouts.protein_per_100g(panel(protein=protein), "14 oz") is None
...
def test_weight_denominated_size_survives_into_item_name():
    ...
    """
    name = sprouts.display_item_name("Grass Fed Ground Beef", "per lb")
...
def test_package_sized_item_also_carries_its_size_into_the_name():
    ...
    """
    name = sprouts.display_item_name("Greek Yogurt", "7 oz")
    assert savings.parse_size(name) is not None
...
def test_the_size_is_not_doubled_when_the_name_already_has_one():
    ...
    name = sprouts.display_item_name("Chicken Breast, 1 lb", "per lb")
...
def test_every_sized_listing_becomes_rankable(product_name, size):
    ...
    """
    name = sprouts.display_item_name(product_name, size)
    assert savings.parse_size(name) is not None
...
def test_missing_hash_raises_rather_than_returning_empty():
    ...
    client = sprouts.SproutsClient(client=object())  # never used -- raises first
    client.hashes = {}
    with pytest.raises(sprouts.QueryNotAllowedError):
...
def test_nutrition_from_payload_reads_a_real_panel():
    ...
    facts = sprouts.nutrition_from_payload(payload)
    assert facts.protein_per_serving == 13.0
    assert sprouts.serving_grams(facts.serving_size) == 227.0
...
def test_missing_panel_is_an_absence_not_an_error():
    ...
    assert sprouts.nutrition_from_payload({"data": {"productNutritionalInfo": {}}}) is None
    assert sprouts.nutrition_from_payload({"data": {}}) is None
...
def test_parse_listing_reads_price_and_size_from_json_ld():
    ...
    listing = sprouts.parse_listing("70516703-sprouts-greek-yogurt-7-oz", head)
...
def test_parse_listing_is_none_when_the_head_was_cut_short():
    ...
    assert sprouts.parse_listing("1-x", "<html><head>" + "x" * 500) is None
...
def test_food_fact_only_written_when_a_density_exists():
    ...
    unusable = sprouts.Nutrition(10.0, "PER CONTAINER", "Varied")
